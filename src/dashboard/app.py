...
# Définition statique des features pour éviter de relire le gros CSV de 400MB via model_preparation
FEATURE_COLS = [
...
st.markdown("Anticipation des pics d'affluence (J+1 à J+4) propulsée par notre Modèle Linéaire (Ridge)")

# SIDEBAR: Correspondant exactement à FEATURE_COLS de model_preparation.py
st.sidebar.header("🛠️ Simulation (Facteurs J-1)")
st.sidebar.markdown("Ces paramètres alimentent directement les 17 features du modèle.")
...
@st.cache_data
def load_data():
    # Génération d'un historique simulé (les données brutes de 400Mo ne sont pas embarquées sur le serveur Cloud)
...
# Lags historiques (ou valeurs de repli)
...
for i, d in enumerate(future_dates):
    ...
    # Création du payload correspondant exactement à FEATURE_COLS
    payload = {
        "month": month,
    ...
    if final_model is not None and scaler is not None:
        # Étape 1 : Scaling avec le StandardScaler exporté
        scaled_payload = scaler.transform(simulated_df)
        if i == 0:
            scaled_payload_j1 = scaled_payload[0]
        # Étape 2 : Prédiction Ridge
        pred_val = float(final_model.predict(scaled_payload)[0])
    else:
        # Fallback fictif en cas d'absence du modèle : on fait réagir TOUS les curseurs
        base = current_lag_1
        base += (sim_temp_max - 20) * 0.8
        base += (sim_severity - 2.5) * 4.0
        base += (sim_ips - 5.0) * 0.5
        base += (sim_age - 45.0) * 0.2
        pred_val = base
...
fig.add_trace(go.Scatter(x=pred_df['date'], y=pred_df['ed_visits'], mode='lines+markers', name='Prédictions (Linear Model)', line=dict(color='#F63366', width=3)))
...
# Graphique dynamique de la contribution locale des variables (SHAP Equivalent)
st.markdown("### ⚖️ Impact Réel des Facteurs (Cascade SHAP)")
import shap
import matplotlib.pyplot as plt
import numpy as np

if final_model is not None and 'scaled_payload_j1' in locals():
    # Pour un modèle linéaire (Ridge), la contribution exacte est coef * valeur_mise_a_l_echelle
    contributions = final_model.coef_ * scaled_payload_j1
    base_value = float(final_model.intercept_)
    
    explanation = shap.Explanation(
        values=contributions,
        base_values=base_value,
        data=simulated_df.iloc[0].values, # Afficher les valeurs saisies non-scalées
        feature_names=FEATURE_COLS
    )
else:
    # Fallback factice si le modèle n'est pas chargé
    base_value = 130.0
    contributions = np.array([12.5, -5.2, 8.4, -3.1, 2.0])
    feature_data = np.array([25.0, 2.5, 45.0, 5.0, 25.0])
    feature_names = ["Température Max", "Sévérité Triage", "Âge Moyen", "Cas IPS", "Score NEMS"]
